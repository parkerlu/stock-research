...
def barrier_labels(px: pd.DataFrame, h: int, bars: list[tuple]) -> pd.DataFrame:
    """三重障碍标签 —— 从【次日开盘真实成交价】起算, 买不到的样本直接剔除。

    ⚠️ v4 的教训(2026-09-07, 差点当成结论交出去):
       标签定义成"20日内能涨15%", 模型就去学【追涨】—— A股里能涨15%的票
       次日多半跳空或一字涨停, 根本买不到。回测算出 1.70, 补上成交约束后
       变成 -0.10; 那 1.70 全部来自 886 笔买不到的一字涨停。

    所以入场必须按【可成交】定义, 且写进标签而不是等到回测才补:
       entry = t+1 开盘价
       若 t+1 一字涨停(最低价已在涨停位) -> 这个样本【丢掉】, 不参与训练
       障碍与收益全部相对 entry 计算, 而不是相对 t 日收盘

    这样模型再也拿不到"买不到的票"的分, 追涨这条捷径就被堵死了。

    cum 用【当日超额】(减全市场当日等权均值), 去 regime 保证不变。
    """
