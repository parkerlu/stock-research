...
def main() -> None:
    ...
    if min_hist > NBAR:
        ...
        valid = valid & enough
    # ⚠️ 阈值写死不自动推导, 是为了不动已跑完的三组(100/200/400)的网络结构 ——
    #    nbar 对照只有在"除输入长度外一切相同"时才可比。加 nbar>=512 这一档时,
    #    深度也跟着变(多一个膨胀块), 所以 800 vs 400 是【配置对比】不是纯单变量,
    #    汇报时要说清楚。
    #    Net 末端是全局平均池化 + 末端取值, 所以感受野小于 nbar 时整段输入仍然
    #    参与(只是每个位置的局部窗口有限), 但让感受野盖住整段仍然更合理。
    if a.nbar >= 512:
        dil = (1, 2, 4, 8, 16, 32, 64, 128, 256)   # 感受野 1027
    elif a.nbar >= 256:
        dil = (1, 2, 4, 8, 16, 32, 64)             # 感受野 259
    else:
        dil = (1, 2, 4, 8, 16, 32)                 # 感受野 131
